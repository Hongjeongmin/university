...
{										// k_count 초기값은 path[m-1][n-1].x; 값 
	...
	//	cout<<"m!= && n==0"<<endl;
		//맨꼭대기에서 왼쪽으로만 가는경우~ 
		ref = Route(m,n-1,k-ref.x);
		ref.x += path[m][n].x;
		if(ref.x == k-path[m][n].x)
		{
	...
	else
	{
		...
		route_m = Route(m,n-1,k-ref.x);
		route_u = Route(m-1,n,k-ref.x);
 
		if(route_m.x== k -path[m][n].x && route_u.x == k-path[m][n].x)
		{
			route_m.weight += path[m][n-1].w_r;
			route_u.weight += path[m-1][n].w_b;

			if(route_m.weight> route_u.weight)
			{
			...
			else if(route_m.weight == route_u.weight)
			{
				for(int i=0;;i++)
				{
